...
for k in range(1,nz+1):
    for j in range(1,ny+1):
        for i in range(1,nx+1):
            ...
            Pi = float(input('What is the value of initial reservoir pressure for layer {0}?'.format(block_n_order)))
            boi = float(input('What is the value of initial oil FVF of layer {0}?'.format(block_n_order)))
