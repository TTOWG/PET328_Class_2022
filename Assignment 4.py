...
boi = float(input('What is the initial oil FVF'))
...
for k in range(1, nz + 1):
    Pi = float(input('What is the value of current reservoir pressure for layer {0}?'.format(k)))
    boi = float(input('What is the initial oil volume factor'. format(k)))
...
print('The total cumulative oil produced from the entire reservoir is {0:.2f} STB'.format(total_np))
