...
for z in range(1, nz+1):
    bo = float(input(
        'What is the value of Bo layer {}?'.format(z)))
    Pi = float(input(
        'What is the value of initial reservoir pressure for layer {}?'.format(z)))
    for j in range(1, ny+1):
        for i in range(1, nx+1):
            block_n_order = (nx*(j-1))+i
            Pnow = float(input(
                'What is the current value of pressure for Block {0} layer {1}?'.format(block_n_order, z)))
            # bo = bob*(1 - (co*(Pnow - Pb)))
