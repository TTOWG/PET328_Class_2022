...
nx = int(input('The absolute number of blocks in x-axis is'))
ny = int(input('The absolute number of blocks in y-axis is'))
nz = int(input('The absolute number of blocks in z-axis is'))
...
for k in range(1,nz+1):
    for j in range(1,ny+1):
         for i in range(1,nx+1):
             ...
             Bo = float(input('What is the value of FVF in layer {0}'.format(k)))
...
print('The cumulative amount of oil produced from the entire reservoir is {0:.2f} STB'.format(total_np))                       
