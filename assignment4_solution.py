...
nz = int(input('How many blocks there are in z-axis?')) # 0.5 marks
...
total_np = 0
# the 'for' loops
for k in range(1,nz+1): # 0.5 marks
    Pi = float(input('What is the value of initial reservoir pressure for Layer {0}?'.format(k)))  # 0.5 marks
    boi = float(input('What is the value of initial oil FVF for Layer {0}?'.format(k)))  # 0.5 marks
    # 0.5 marks for placing these lines before the j and i loops
    for j in range(1,ny+1):
        for i in range(1,nx+1):
            block_n_order = (nx*ny*(k-1)) + (nx*(j-1)) + i   # 0.5 marks
